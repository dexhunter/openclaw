...
const successorRotationPrefixCarryMarkers = [...successorRotationPrefixCarryTypes].map(
  (type) => `"${type}"`,
);
...
async function hasSuccessorRotationCarryEntryBefore(
  ...
  while (position < end) {
    ...
    for (const line of lines) {
      if (lineHasSuccessorRotationCarryEntry(line)) {
        return true;
      }
    }
  }
...
function lineHasSuccessorRotationCarryEntry(line: string): boolean {
  if (!successorRotationPrefixCarryMarkers.some((marker) => line.includes(marker))) {
    return false;
  }
  try {
    const entry: unknown = JSON.parse(line);
    return isRecord(entry) && successorRotationPrefixCarryTypes.has(String(entry.type));
  } catch {
    return true;
  }
}
